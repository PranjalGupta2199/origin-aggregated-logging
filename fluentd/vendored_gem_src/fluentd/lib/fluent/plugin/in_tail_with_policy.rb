...
module Fluent::Plugin
  class TailInput < Fluent::Plugin::Input
    Fluent::Plugin.register_input('tail_with_policy', self)
    ...
    desc 'Group rate limits (lines/sec)'
    config_param :group_line_limit, :string, default: ''
    ...
    def start
      ...
      @paths.each_with_index { |path, idx|
        ...
        if idx < @group_line_limits.size
          begin
            lim = Integer(@group_line_limits[idx])
    ...
    def update_watcher(path, pe, gw)
      ...
      @tails[path] = setup_watcher(path, pe, gw)
    ...
    class GroupWatcher
      ...
      def initialize(path, group_line_limit = -1)
        @group_path = path
        @current_paths = []
        @group_line_limit = group_line_limit        # total per second line limit of files in group
        @number_lines_read = 0
    ...
    class TailWatcher
      def initialize(path, rotate_wait, pe, log, read_from_head, enable_watch_timer, enable_stat_watcher, read_lines_limit, write_watcher, update_watcher, line_buffer_timer_flusher, from_encoding, encoding, open_on_every_update, gw, &receive_lines)
        @path = path
      ...
      def on_rotate(stat)
        if @io_handler.nil?
          if stat
            ...
            @io_handler = IOHandler.new(self,@write_watcher, @group_watcher, &method(:wrap_receive_lines))
          else
          ...
          if watcher_needs_update
            @update_watcher.call(@path, swap_state(@pe), @group_watcher)
          else
            @io_handler = IOHandler.new(self, @write_watcher, @group_watcher, &method(:wrap_receive_lines))
          end
        end
      ...
      class IOHandler
        def initialize(watcher, write_watcher, gw, &receive_lines)
          @watcher = watcher
          @group_watcher = gw
          @write_watcher=write_watcher
          @receive_lines = receive_lines
